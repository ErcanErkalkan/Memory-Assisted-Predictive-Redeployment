...
def transfer_candidates(memory_hist: List[np.ndarray], N: int, D: int, rng: np.random.Generator, cfg: Config) -> np.ndarray:
    ...
    if not valid:
        return rng.random((N, D))
    # Bias transfer toward recent archives but keep earlier environments available.
...
def run_ablation(cfg: Config, outdir: Path) -> None:
    ...
    A compact five-scenario representative diagnostic previously wrote
    generic files such as ablation_summary.csv. To avoid ambiguity, the
    manuscript-facing ablation covers all 20 scenarios and writes only files with
    the ablation_all20_ prefix. Five-scenario provenance outputs, if retained,
    should be kept outside the primary results directory.
    """
...
def run_prr_source_diagnostics(cfg: Config, outdir: Path) -> None:
    ...
    with open(outdir / "README_PRR_SOURCE_DIAGNOSTICS.txt", "w") as f:
        f.write(
            ...
            "Run example from 04_supplementary_material/src:\n"
